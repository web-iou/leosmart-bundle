...
declare global {
  namespace ReactNavigation {
    interface RootStackParamList {
      DeviceControl: DeviceControlParams;
    }
  }
}
...
const DeviceControl = ({ 
  ...
  // 获取设备信息
  useEffect(() => {
    // 如果需要，可以根据deviceId加载设备详细信息
    console.log(`加载设备ID: ${deviceId} 的信息`);
  ...
  }, [navigation, t, theme.colors.background, theme.colors.onBackground, deviceName]);
  ...
  const handleRemoteShutdownToggle = async (value: boolean) => {
    try {
      ...
      const { data: success } = await deviceApi.setEquipmentParams(deviceSn, command);
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
            ...
            <List.Item
              ...
              description={t('device.adjustOutputPower', { defaultValue: '调节设备输出功率' })}
              left={_props => (
            ...
            <Divider style={{ backgroundColor: theme.dark ? 'rgba(255, 255, 255, 0.1)' : theme.colors.outlineVariant }} />
            
            {/* 远程开关机 */}
            <List.Item
              title={t('device.remoteShutdown', { defaultValue: '远程开关机' })}
              description={t('device.remoteShutdownDesc', { defaultValue: '远程控制设备开关' })}
              left={_props => (
                <View style={[styles.iconContainer, { 
                  backgroundColor: theme.dark ? theme.colors.surfaceVariant : theme.colors.primaryContainer 
                }]}>
              ...
              right={_props => (
                <Switch
                  ...
                  disabled={loading}
                  trackColor={{ 
                    false: theme.dark ? 'rgba(255, 255, 255, 0.15)' : theme.colors.outlineVariant, 
                    true: theme.colors.primary 
                  }}
                  thumbColor={remoteShutdownEnabled ? 
                    theme.colors.onPrimary : 
                    theme.dark ? theme.colors.onSurfaceVariant : theme.colors.surface}
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
            ...
            <List.Item
              ...
              description={t('device.factoryResetDesc', { defaultValue: '重置所有数据及设置' })}
              left={_props => (
                <View style={[styles.iconContainer, { 
                  backgroundColor: theme.dark ? theme.colors.surfaceVariant : theme.colors.primaryContainer 
                }]}>
              ...
              onPress={() => {
                Alert.alert(
                  ...
                  t('device.factoryResetConfirm', { defaultValue: '确定要恢复出厂设置吗？此操作不可逆。' }),
                  [
                    {
                    ...
                    },
                    {
                      ...
                      onPress: () => {
                        // TODO: 实现恢复出厂设置逻辑
                        Alert.alert(
                          t('common.notImplemented', { defaultValue: '功能未实现' }),
                          t('common.comingSoon', { defaultValue: '即将推出，敬请期待' })
                        );
                      },
                    },
            ...
            <List.Item
              ...
              description={t('device.securityInfoDesc', { defaultValue: '查看设备安全认证信息' })}
              left={_props => (
                <View style={[styles.iconContainer, { 
                  backgroundColor: theme.dark ? theme.colors.surfaceVariant : theme.colors.primaryContainer 
                }]}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
            <View style={styles.infoRow}>
              <Text style={[styles.infoLabel, { color: theme.dark ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.6)' }]}>

...
export const fetchSupportedLanguages = async (): Promise<LanguageItem[]> => {
  try {
    ...
    // 如果有缓存的语言列表则使用缓存
    try {
      const cachedLanguages = await storage.getStringAsync('supportedLanguages');
      if (cachedLanguages) {
        try {
...
export const fetchTranslation = async (lang: string): Promise<any> => {
  try {
    ...
    try {
      const timestampStr = await storage.getStringAsync(`translations_${lang}_timestamp`);
      if (timestampStr) {
        ...
        if (now - timestamp < 3600000) {
          shouldFetch = false;
          console.log(`Cache for ${lang} is still fresh (less than 1 hour old)`);
        }
      }
    } catch (e) {
    ...
    if (response.code === 0 && response.data) {
      ...
      // 缓存翻译数据
      await storage.setAsync(`translations_${lang}`, JSON.stringify(translations));
      await storage.setAsync(`translations_${lang}_timestamp`, Date.now().toString());
...
export const initializeI18n = async () => {
  try {
    ...
    try {
      currentLang = await storage.getStringAsync('language') || APP_SETTINGS.defaultLanguage;
      console.log(`Current language from storage: ${currentLang}`);
    } catch (error) {
      console.warn('Failed to get language from storage, using default:', error);
